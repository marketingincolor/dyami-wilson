...
?>
      ...
      <?php }else if ($count_faqs == 2) { ?>
	  			...
	  			<div class="small-12 medium-6 columns end" style="margin-bottom:80px;">
          ...
          <?php }else if ($count_faqs >= 3) { ?>
	  			  ...
	  			  <div class="small-12 medium-6 large-4 columns end" style="margin-bottom:80px">

								<?php if (has_post_format('audio')) { ?>

			  				<div class="feat-img-cont vertical shadow" data-audio="<?php the_field('file'); ?>" data-open="audio-video-modal" data-title="<?php the_title(); ?>" style="background-image: linear-gradient(rgba(0,0,0,0.3), rgba(0,0,0,0.3)),url(<?php the_post_thumbnail_url(); ?>);">
		  						<div class="play">
		  							<p><i class="fa fa-volume-up" aria-hidden="true"></i></p>
		  							<p>Play Audio</p>
		  						</div>
			  				...
			  				<div class="feat-img-cont vertical shadow" data-video="<?php the_field('vimeo_link'); ?>" data-open="audio-video-modal" data-title="<?php the_title(); ?>" style="background-image: linear-gradient(rgba(0,0,0,0.3), rgba(0,0,0,0.3)),url(<?php the_post_thumbnail_url(); ?>);">
		  						<div class="play">
		  							<p><i class="fa fa-play-circle" aria-hidden="true"></i></p>
		  							<p>Play Video</p>
		  						</div>
	  				...
	  				<h3><?php the_title(); ?></h3>
	  				<p><?php if(get_field('author')){ echo 'by ';the_field('author');}else{echo '<em>(Re-creation of Original)</em>';} ?></p>
	  				<p class="description">
	  					<?php echo mb_strimwidth(get_the_content(), 0, 150, '...'); ?>
	  				</p>
	  				<a href="<?php the_permalink(); ?>"><button class="hvr-rectangle-out btn-black">View Project</button></a>
